    ...
    public static ArrayList<ChessMove> calculatePawnMoves(ChessBoard board, ChessPosition pos) {
        ...
        if (piece.getTeamColor() == ChessGame.TeamColor.WHITE) {
            if (row != 7) {
                ...
                if (col > 1 && board.getPiece(new ChessPosition(row + 1, col - 1)) != null &&
                        board.getPiece(new ChessPosition(row + 1, col - 1)).getTeamColor() == ChessGame.TeamColor.BLACK) {
                    moves.add(new ChessMove(pos, new ChessPosition(row + 1, col - 1), null));
                }

                if (col < 8 && board.getPiece(new ChessPosition(row + 1, col + 1)) != null &&
                        board.getPiece(new ChessPosition(row + 1, col + 1)).getTeamColor() == ChessGame.TeamColor.BLACK) {
                    moves.add(new ChessMove(pos, new ChessPosition(row + 1, col + 1), null));
                }
            ...
            if (row != 2) {
                ...
                if (col > 1 && board.getPiece(new ChessPosition(row - 1, col - 1)) != null &&
                        board.getPiece(new ChessPosition(row - 1, col - 1)).getTeamColor() == ChessGame.TeamColor.WHITE) {
                    moves.add(new ChessMove(pos, new ChessPosition(row - 1, col - 1), null));
                }
